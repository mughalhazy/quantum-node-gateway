...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!ALLOW_WHM.has("suspendacct")) return res.status(403).json({ ok:false, error:"not_allowed" });

    const { user, reason } = JSON.parse(raw);
    if (!user) return res.status(400).json({ ok:false, error:"user_required" });

    const data = await whmJson("suspendacct", { user, reason });
    return res.json({ ok:true, result: data?.data || data });
  } catch (e) {
    console.error("[whm:suspendacct]", e?.message || e);
